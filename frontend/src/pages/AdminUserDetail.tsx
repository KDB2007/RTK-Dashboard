...
export function AdminUserDetail() {
  ...
  useEffect(() => {
    ...
    const fetch = () => {
      Promise.all([
        ...
        api.dashboard.commands(userId),
        api.dashboard.trends(30, userId),
        api.dashboard.history(50, 0, userId),
      ])
        .then(([s, c, t, h]) => {
          setSummary(s);
          setCommands(c);
  ...
  return (
    <Layout>
      <Link to="/admin" className="text-sky-400 hover:text-sky-300 text-sm mb-4 inline-block">&larr; Back to all users</Link>
